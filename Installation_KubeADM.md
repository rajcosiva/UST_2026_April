...
# Verify that the br\_netfilter, overlay modules are loaded by running the following commands:
lsmod | grep br\_netfilter
lsmod | grep overlay

# Verify that the net.bridge.bridge-nf-call-iptables, net.bridge.bridge-nf-call-ip6tables, and net.ipv4.ip\_forward system variables are set to 1 in your sysctl config by running the following command:
sysctl net.bridge.bridge-nf-call-iptables net.bridge.bridge-nf-call-ip6tables net.ipv4.ip\_forward
```
...
`kubectl set env daemonset/calico-node -n calico-system IP\_AUTODETECTION\_METHOD=interface=ens5`
Where ens5 is your default interface, you can confirm by running `ifconfig` on all the machines
* IP\_AUTODETECTION\_METHOD  is set to first-found to let Calico automatically select the appropriate interface on each node.
